...
export class WorkflowFactory {
  ...
  async createWorkflowFromYAML(
    ...
    for (const task of workflow.tasks) {
      const yamlStep = workflowDef.steps.find(
        (s) => s.stepNumber === task.stepNumber
      );
      if (yamlStep?.dependsOn !== undefined) {
        const dependencyTask = workflow.tasks.find(
          (t) => t.stepNumber === yamlStep.dependsOn
        );
        if (dependencyTask) {
          task.dependsOn = dependencyTask.taskId;
        }
      }
    }
